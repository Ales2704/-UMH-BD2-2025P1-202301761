...
BEGIN  
    ...
    select iso_code into v_iso_code from mi_base_datos.currencies 
...
CALL mi_base_datos.sp_new_currency_case(
	"COLON CONSTARRICENSE", "c",   0,  "COSTA RICA" , "CRC"
); 
 

select * from  mi_base_datos.currencies where iso_code = 'CRC';

set @isovar = "";
select iso_code into @isovar from mi_base_datos.currencies where iso_code = 'CRC';
select @isovar;
